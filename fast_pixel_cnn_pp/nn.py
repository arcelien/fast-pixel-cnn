...
def num_mult(x):
    channels = list(map(int, x.get_shape()))[3]
    if channels == 1:
        return 1 + 1*2, 2, 1
    elif channels == 2:
        return 1 + 2*3, 3, 2
...
dim_mult_1, dim_mult_2, channels = 1 + 2*3, 3, 2
def discretized_mix_logistic_loss(x,l,sum_all=True):
    """ log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval """
    xs = int_shape(x) # true image (i.e. labels) to regress to, e.g. (B,32,32,3)
    ls = int_shape(l) # predicted distribution, e.g. (B,32,32,100)
    nr_mix = int(ls[-1] / dim_mult_1) # here and below: unpacking the params of the mixture of logistics
    print('l shape', ls, 'mix', nr_mix)

    logit_probs = l[:,:,:,:nr_mix]
    l = tf.reshape(l[:,:,:,nr_mix:], xs + [nr_mix*dim_mult_2])
    means = l[:,:,:,:,:nr_mix]
